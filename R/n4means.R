...
{
nTemp <- r$n;
r$n <- 2*sigma^2*(1 + (m - 1)*ICC)*(qt((1 - alpha/2), df=(nTemp - 2)) + qt(power, df=(nTemp - 2)))^2/(m*(delta^2));
}
...
{
nTemp <- r$n;
r$n <- 2*sigma^2*(1 + (m - 1)*ICC)*(qt((1 - alpha), df=(nTemp - 2)) + qt(power, df=(nTemp - 2)))^2/(m*(delta^2));
}
