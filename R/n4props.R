...
{
nTemp <- r$n;
r$n <- ((qt((1 - alpha/2), df=(nTemp -2)) + qt(power, df=(nTemp -2)))^2*(pe*(1-pe) + pc*(1-pc))*(1 + (m - 1)*ICC))/(m*(pe - pc)^2);
}
...
{
nTemp <- r$n;
r$n <- ((qt((1 - alpha), df=(nTemp -2)) + qt(power, df=(nTemp -2)))^2*(pe*(1-pe) + pc*(1-pc))*(1 + (m - 1)*ICC))/(m*(pe - pc)^2);
}
